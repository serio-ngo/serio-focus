...
| No edit, write or search coverage. The `PreToolUse` matcher admits `Read`, `Bash`, `PowerShell`, spawn tools, and `WebSearch`/`WebFetch` (subagent call cap only). | `Write`, `Edit`, `Grep`, `Glob` | Allowed. |
